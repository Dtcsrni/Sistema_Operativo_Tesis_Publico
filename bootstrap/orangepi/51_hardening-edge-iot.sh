...
Cmnd_Alias TESIS_EDGE_MAINT = /usr/bin/systemctl status edge-iot.service, /usr/bin/systemctl restart edge-iot.service, /usr/bin/systemctl status fail2ban, /usr/bin/systemctl restart fail2ban, /usr/sbin/ufw status, /usr/sbin/ufw reload
${admin_user} ALL=(root) NOPASSWD: TESIS_EDGE_MAINT
EOF

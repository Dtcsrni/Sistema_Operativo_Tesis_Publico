...
- Página anterior en la ruta base: [Arquitectura](../publico/wiki/arquitectura.md).
...
- La capa canónica no pública puede mostrar ejemplos concretos como validación humana interna no pública o evento interno no público.
...
- Objetivo: Nombres autodescriptivos, legibles por humanos, estables para versionado e ingestión, evitando siglas opacas salvo que sean ampliamente reconocidas en el proyecto.
- Patrón base: `{ambito}__{descripcion_del_contenido}__{forma_o_proposito}__{version}.{extension}`

### Reglas activas

- Usar minúsculas.
- Usar palabras completas en español cuando sea posible.
- Separar bloques semánticos con doble guion bajo (__).
- Evitar siglas no explicadas; permitir solo IoT, APA, Overleaf y SQLite cuando el contexto ya sea inequívoco.
- Conservar la versión al final antes de la extensión.
- Preferir nombres orientados a función: archivo_de_contexto, registro_estructurado, catalogo_general_de_artefactos.

...
- Plano remoto activo: `Telegram` para notificaciones, compatibilidad y fallback.
- `Matrix` queda como capacidad latente y no forma parte del camino diario hasta nueva decisión humana.
...
4. En Orange Pi, revisar estado edge:
   ...
   - `systemctl status openclaw-matrix-bot.service` si Matrix se habilita explícitamente
...
- Si el cambio toca OpenClaw remoto, reiniciar tambien los servicios `openclaw-gateway` y `openclaw-telegram-bot`; `openclaw-matrix-bot` solo si Matrix se habilita de forma expresa.

...
## Objetivo
Ofrecer una superficie híbrida `CLI + web + Telegram` para operar `openclaw` con política `desktop-first`: la PC principal resuelve el carril pesado y el edge conserva continuidad 24/7. Matrix queda como capacidad latente, no como canal operativo diario.

## Flujo mínimo
1. Ejecutar `python runtime/openclaw/bin/openclaw_local.py doctor`.
2. Verificar nodos y proveedores efectivos con `pasarela estado`, `telegram estado`, `proveedor estado` y `presupuesto estado`.
...
- Cuando una tarea tiene `target_paths`, contexto documental, riesgo alto o modo académico, el CLI usa Serena como requisito operativo por defecto.
...
- Si Serena no está disponible, `OpenClaw` intenta levantar `serena-local` por HTTP mediante `07_scripts/check_serena_access.py --attempt-start-http`. Si después de eso sigue sin responder, el comando falla cuando el modo sea `required`.
...
## Router agéntico y nubes gratuitas
- `07_scripts/agent_task_router.py classify` decide ruta por privacidad, riesgo, complejidad, rutas objetivo y necesidad de documentación externa.
- OpenClaw prioriza routers ya existentes y documentados: su `route_task` gobernado se mantiene como contrato interno; los runtimes locales se consumen por APIs proveedor de IA no publicado-compatible documentadas (`Ollama` y `llama.cpp`); y un router externo (`LiteLLM`, `OpenRouter` u otro proveedor de IA no publicado-compatible) solo entra por `external_llm_router` cuando `OPENCLAW_EXTERNAL_ROUTER_ENABLED=1`.
- `external_llm_router` no sustituye gates, presupuesto, privacidad ni trazabilidad de OpenClaw. Solo delega selección/fallback de modelo a un gateway documentado cuando el dominio lo permite y hay URL/API key explícitas.
- Por seguridad de supply chain, no instalar routers Python sin versión revisada y fijada. Para LiteLLM se prefiere proxy/imagen oficial o dependencia pinneada fuera del camino crítico del edge.
- `OPENCLAW_ADAPTIVE_ROUTING_ENABLED=0` conserva por defecto el orden existente. Al habilitarlo, el overlay de benchmark solo reordena candidatos y bloquea autopromoción NPU; no pisa feedback reciente ni peticiones explícitas de ChatGPT/web-session.
- Modelos locales pueden ejecutar subtareas automáticas, pero no escriben directo al repo; el agente principal integra después de gates.
- `context7_docs` se reserva para documentación externa actualizada y versionada.
- `github_models_free` solo puede recibir contexto `public` o `redacted`, requiere token con `models:read` y está bloqueado por defecto para evidencia privada, secretos, ledger privado, canon no público y rutas sensibles.
- `07_scripts/ab_pilot.py` compara rutas agénticas por tokens, costo, latencia, aceptación, fallos de gate y retrabajo.

## Configuración del adapter Serena
- `OPENCLAW_SERENA_ENABLED=1|0`: activa o desactiva la heurística automática.
- `OPENCLAW_SERENA_MODE=required|auto|off`: política del adapter interno; la plantilla reproducible usa `auto` y los comandos críticos pueden exigir `required`.
...
- `OPENCLAW_SERENA_AUTO_START=1`: permite que OpenClaw intente restaurar el HTTP local antes de fallar.
...
- Default recomendado para plantillas reproducibles: `OPENCLAW_SERENA_ENABLED=1`, `OPENCLAW_SERENA_MODE=auto`, `OPENCLAW_SERENA_TRANSPORT=http`, `OPENCLAW_SERENA_AUTO_START=1`. La política AGENTS sigue exigiendo comprobar/restaurar Serena antes de degradar a filesystem.
...
- La pasarela expone una `session-layer` común para `cli`, `web_local` y `telegram`. `matrix` permanece como extensión latente, con estado `not_configured` por defecto.
...
## Fuentes APA y evidencia
- El gestor local de fuentes usa JSONL append-only como evidencia primaria (`OPENCLAW_SOURCE_JSONL`) y SQLite como espejo operativo (`source_records`).
- CLI:
...
- La verificación DOI usa Crossref REST con polite pool cuando `OPENCLAW_CROSSREF_MAILTO` está configurado. Si DOI/URL/hash local fallan, la fuente queda `no_verificable` y no debe respaldar afirmaciones fuertes.
- La salida APA se renderiza como APA 7 operativo; no equivale a validación humana de una cita final de tesis.

## Voz por turnos
- Telegram puede transcribir y responder con audio cuando hay STT/TTS configurado.
- Defaults de plantilla: `OPENCLAW_TELEGRAM_STT_MODEL=gpt-4o-mini-transcribe`, `OPENCLAW_TELEGRAM_TTS_MODEL=gpt-4o-mini-tts`.
- Contenido privado o sensible debe quedarse local o pasar por redacción antes de usar STT/TTS de nube.
...
## Telegram como canal remoto activo
- `openclaw-telegram-bot.service` se mantiene para control remoto, notificaciones y fallback operativo.
- Telegram comparte la misma `session-layer` y persiste sesiones/mensajes como canal activo.
...
## Matrix latente
- `OPENCLAW_MATRIX_ENABLED=0` es el valor esperado por defecto.
- El bot/adaptador Matrix y `matrix_bot.py` permanecen como capacidad futura.
- Variables mínimas para futura activación:
  - `OPENCLAW_MATRIX_HOMESERVER`
  - `OPENCLAW_MATRIX_ACCESS_TOKEN`
  - `OPENCLAW_MATRIX_USER_ID`
  - `OPENCLAW_MATRIX_ROOM_IDS`
- Comandos disponibles cuando se habilite explícitamente:
...
- El homeserver Matrix se puede alojar inicialmente en `tesis-edge` o migrar a VPS sin cambiar el contrato, pero esa ruta no forma parte del flujo diario actual.
...
- La herramienta de imagen queda deshabilitada en el contrato operativo activo. `OPENCLAW_IMAGE_BACKEND=disabled` evita crear APR ejecutables de imagen y responde con `image_generation_disabled`.
...
- El túnel PC -> edge se mantiene con `openclaw-desktop-tunnel.service` en la PC: `127.0.0.1:21434` del edge apunta al `llama.cpp server` de Windows en `172.17.176.1:21435` cuando `OPENCLAW_DESKTOP_TUNNEL_LOCAL_BIND=172.17.176.1:21435`.

...
from openclaw_local.adaptive_router import build_adaptive_routing_snapshot  # noqa: E402
from openclaw_local.toltecayotl_ingestor import ingest_document  # noqa: E402
from openclaw_local.toltecayotl_knowledge import toltecayotl_status, search_knowledge, upsert_chunks  # noqa: E402
...
from openclaw_local.knowledge_sync import default_sync_dir, export_sync_package, import_sync_package  # noqa: E402
...
from openclaw_local.sources import append_reference_jsonl, default_source_jsonl, ingest_reference, source_policy_snapshot  # noqa: E402
...
from openclaw_local.telegram_bot import dispatch_command, handle_update, run_polling_loop  # noqa: E402
...
def cmd_doctor(_: argparse.Namespace) -> int:
    ...
    payload = {
        ...
        "adaptive_routing": build_adaptive_routing_snapshot(ROOT, store=store).to_dict(),
        ...
        warning = _safe_store_write(
            ...
            lambda item=item: store.save_benchmark_record(
                BenchmarkRecord(
                    ...
                    primary_jsonl=str(item.get("primary_jsonl", "")),
                    scientific_validity=str(item.get("scientific_validity", "")),
...
def cmd_sources_status(args: argparse.Namespace) -> int:
    store = _store()
    payload = source_policy_snapshot(ROOT, store=store, limit=args.limit)
    payload["store"] = store.audit_summary()
    print(json.dumps(payload, ensure_ascii=False, indent=2))
    return 0
...
def cmd_sources_ingest(args: argparse.Namespace) -> int:
    ...
    record = ingest_reference(
        ...
        title=args.title,
        authors=[item.strip() for item in (args.authors or []) if item.strip()],
        year=args.year,
        doi=args.doi,
        url=args.url,
        ...
        claims=[item.strip() for item in (args.claims or []) if item.strip()],
        tags=[item.strip() for item in (args.tags or []) if item.strip()],
        verify_online=not args.no_online_verify,
    )
    store.save_reference_record(record)
    jsonl_path = Path(args.jsonl) if args.jsonl else default_source_jsonl(ROOT)
    append_reference_jsonl(record, jsonl_path)
    print(
        json.dumps(
            {
                "status": "ok",
                ...
                "note": "Registro de fuente creado; no equivale a validacion humana de afirmaciones.",
            },
            ensure_ascii=False,
            indent=2,
        )
...
def cmd_toltecayotl_status(_: argparse.Namespace) -> int:
    print(json.dumps(toltecayotl_status(ROOT), ensure_ascii=False, indent=2))
    return 0
...
def cmd_toltecayotl_ingest(args: argparse.Namespace) -> int:
    ...
    storage = upsert_chunks(ROOT, chunks)
    print(
        json.dumps(
            {
                "status": "ok",
                ...
                "storage": storage,
                "note": "Ingesta Toltecayotl creada con hashes; no equivale a validacion humana de afirmaciones.",
            },
            ensure_ascii=False,
            indent=2,
        )
...
def cmd_toltecayotl_search(args: argparse.Namespace) -> int:
    print(json.dumps(search_knowledge(ROOT, args.query, limit=args.limit), ensure_ascii=False, indent=2))
    return 0


def cmd_toltecayotl_sync_export(args: argparse.Namespace) -> int:
    destination = Path(args.output) if args.output else None
    package = export_sync_package(ROOT, destination=destination)
    print(json.dumps({"status": "ok", "package": package.to_dict(), "sync_dir": str(default_sync_dir(ROOT))}, ensure_ascii=False, indent=2))
    return 0


def cmd_toltecayotl_sync_import(args: argparse.Namespace) -> int:
    payload = import_sync_package(ROOT, Path(args.file))
    print(json.dumps(payload, ensure_ascii=False, indent=2))
    return 0
...
def cmd_telegram_reply(args: argparse.Namespace) -> int:
    """Procesa un comando de telegram con manejo robusto de encoding."""
    store = _store()
    
    # Asegurar que los argumentos están correctamente decodificados en UTF-8
    command = str(args.command or "").strip()
    text = str(args.text or "").strip()
    chat_id = str(args.chat_id or "").strip()
    
    # Normalizar a UTF-8 en caso de problemas de encoding
    ...
    try:
        payload = dispatch_command(command, text, repo_root=ROOT, store=store, chat_id=chat_id)
    except UnicodeEncodeError as exc:
        payload = {
            "status": "error",
            "error": f"Encoding error: {exc}",
            "command": command,
            "text": text[:100],  # Primeros 100 caracteres para debugging
        }
    ...
    # Serializar a JSON con UTF-8 explícito
    try:
        output = json.dumps(payload, ensure_ascii=False, indent=2)
        print(output)
    except Exception as exc:
        # Si hay error al serializar, enviar error message
        error_payload = {"status": "error", "error": f"JSON serialization error: {exc}"}
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sources = subparsers.add_parser("fuentes", aliases=["sources"])
    sources_sub = sources.add_subparsers(dest="sources_command", required=True)
    sources_status = sources_sub.add_parser("estado", aliases=["status"])
    sources_status.add_argument("--limit", type=int, default=10)
    sources_status.set_defaults(func=cmd_sources_status)
    sources_ingest = sources_sub.add_parser("registrar", aliases=["ingest"])
    sources_ingest.add_argument("--tipo", "--type", dest="source_type", default="article")
    sources_ingest.add_argument("--titulo", "--title", dest="title", default="")
    sources_ingest.add_argument("--autor", "--author", dest="authors", action="append", default=[])
    sources_ingest.add_argument("--anio", "--year", dest="year", default="")
    sources_ingest.add_argument("--doi", dest="doi", default="")
    sources_ingest.add_argument("--url", dest="url", default="")
    sources_ingest.add_argument("--archivo", "--file", dest="local_path", default="")
    sources_ingest.add_argument("--editorial", "--publisher", dest="publisher", default="")
    sources_ingest.add_argument("--contenedor", "--container-title", dest="container_title", default="")
    sources_ingest.add_argument("--nivel-evidencia", "--evidence-level", dest="evidence_level", default="pendiente")
    sources_ingest.add_argument("--afirmacion", "--claim", dest="claims", action="append", default=[])
    sources_ingest.add_argument("--tag", dest="tags", action="append", default=[])
    sources_ingest.add_argument("--sin-red", "--no-online-verify", dest="no_online_verify", action="store_true", default=False)
    sources_ingest.add_argument("--jsonl", dest="jsonl", default="")
    sources_ingest.set_defaults(func=cmd_sources_ingest)

    toltecayotl = subparsers.add_parser("toltecayotl")
    toltecayotl_sub = toltecayotl.add_subparsers(dest="toltecayotl_command", required=True)
    toltecayotl_status_parser = toltecayotl_sub.add_parser("estado", aliases=["status"])
    toltecayotl_status_parser.set_defaults(func=cmd_toltecayotl_status)
    toltecayotl_ingest = toltecayotl_sub.add_parser("ingestar", aliases=["ingest"])
    toltecayotl_ingest.add_argument("--archivo", "--file", dest="file", required=True)
    toltecayotl_ingest.add_argument("--tipo", "--type", dest="source_type", required=True, choices=["pdf", "markdown", "latex", "jsonl"])
    toltecayotl_ingest.add_argument("--chunk-chars", dest="chunk_chars", type=int, default=1800)
    toltecayotl_ingest.set_defaults(func=cmd_toltecayotl_ingest)
    toltecayotl_search = toltecayotl_sub.add_parser("buscar", aliases=["search"])
    toltecayotl_search.add_argument("--query", required=True)
    toltecayotl_search.add_argument("--limit", type=int, default=5)
    toltecayotl_search.set_defaults(func=cmd_toltecayotl_search)
    toltecayotl_sync = toltecayotl_sub.add_parser("sync")
    toltecayotl_sync_sub = toltecayotl_sync.add_subparsers(dest="toltecayotl_sync_command", required=True)
    toltecayotl_sync_export = toltecayotl_sync_sub.add_parser("exportar", aliases=["export"])
    toltecayotl_sync_export.add_argument("--salida", "--output", dest="output", default="")
    toltecayotl_sync_export.set_defaults(func=cmd_toltecayotl_sync_export)
    toltecayotl_sync_import = toltecayotl_sync_sub.add_parser("importar", aliases=["import"])
    toltecayotl_sync_import.add_argument("--archivo", "--file", dest="file", required=True)
    toltecayotl_sync_import.set_defaults(func=cmd_toltecayotl_sync_import)

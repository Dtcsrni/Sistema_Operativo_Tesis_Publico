...
class OpenClawStore:
        ...
        with self._connect() as conn:
            conn.executescript(
                ...
                CREATE TABLE IF NOT EXISTS benchmark_runs (
                    ...
                    primary_jsonl TEXT NOT NULL DEFAULT '',
                    scientific_validity TEXT NOT NULL DEFAULT '',
                    payload_json TEXT NOT NULL,
                    created_at TEXT NOT NULL
                );
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                INSERT OR REPLACE INTO benchmark_runs(
                    benchmark_id, run_id, provider, model, status, latency_ms,
                    payload_hash, primary_jsonl, scientific_validity, payload_json, created_at
                ) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    record.benchmark_id,
                    ...
                    record.latency_ms,
                    record.payload_hash,
                    record.primary_jsonl,
                    record.scientific_validity,
                    json.dumps(record.to_dict(), ensure_ascii=False),
                    record.created_at,
                ),
            )
    ...
    def get_request_trace(self, trace_id: str) -> dict[str, Any] | None:
        with self._connect() as conn:
            row = conn.execute("SELECT payload_json FROM request_traces WHERE trace_id = ?", (trace_id,)).fetchone()
        if row is None:
            return None
        return json.loads(row["payload_json"])
    ...
    def save_reference_record(self, record: ReferenceRecord) -> None:
        with self._connect() as conn:
            conn.execute(
                """
                INSERT OR REPLACE INTO source_records(
                    reference_id, source_type, verification_status, doi, url,
                    apa_reference, source_hash, payload_json, created_at
                ) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    record.reference_id,
    ...
    def list_reference_records(self, *, limit: int = 50) -> list[dict[str, Any]]:
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT payload_json FROM source_records ORDER BY created_at DESC LIMIT ?",
                (limit,),
            ).fetchall()
        return [json.loads(row["payload_json"]) for row in rows]

    def get_reference_record(self, reference_id: str) -> dict[str, Any] | None:
        with self._connect() as conn:
            row = conn.execute("SELECT payload_json FROM source_records WHERE reference_id = ?", (reference_id,)).fetchone()
        if row is None:
            return None
        return json.loads(row["payload_json"])

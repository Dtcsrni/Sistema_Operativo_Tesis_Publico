...
def parse_channel_command(text: str) -> tuple[str, str]:
    ...
    if not cleaned.startswith("/"):
        ...
        for prefix in ("investiga", "investigar", "investigue"):
            if normalized == prefix:
                return "investiga", ""
            if normalized.startswith(f"{prefix} "):
                return "investiga", cleaned[len(prefix) :].strip(" ¿?¡!")
        if normalized in {"investigalo", "investigarla", "investigarlo", "lo anterior"}:
            return "investiga", cleaned
        return "chat", cleaned
...
def build_nodes_summary(repo_root: Path) -> list[dict[str, Any]]:
    ...
    edge_base = os.getenv("OPENCLAW_EDGE_INFERENCE_BASE_URL", os.getenv("OPENCLAW_EDGE_OLLAMA_BASE_URL", "http://127.0.0.1:11434")).strip()
    edge_runtime = "edge_inference" if runtime.get("edge", {}).get("ready") else "local"

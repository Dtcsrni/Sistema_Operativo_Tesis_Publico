...
def _provider_measured_candidates(repo_root: Path, provider_id: str, *, request_kind: str) -> list[str]:
    ...
    if provider_id in {"desktop_compute", "pc_native_llamacpp", "llamacpp_local"}:
        if request_kind == "coding":
            ...
            append(measured.get("coding_heavy"))
            append(measured.get("alternative_ceiling"))
    elif provider_id == "edge_inference":
...
def _select_available_model(base_url: str, candidates: list[str], *, provider: str = "edge_inference") -> str:
    if provider in {"llamacpp_local", "pc_native_llamacpp"}:
...
def _chat_backend_candidates(repo_root: Path, profile: dict[str, str]) -> list[ChatBackendCandidate]:
    ...
    edge_timeout = _model_timeout_for(request_kind, complexity, backend="edge_inference")
    ...
    if maestro_chain_raw:
        ...
        for chain_item in maestro_chain:
            ...
            elif provider in {"edge_inference", "ollama_local"} and edge_allowed:
                maestro_candidates.append(ChatBackendCandidate("edge_inference", edge_base, model, edge_timeout, "maestro_local"))
    ...
    if request_kind == "standard" and complexity == "low":
        for model in edge_candidates:
            candidates.append(ChatBackendCandidate("edge_inference", edge_base, model, edge_timeout, "edge_fast"))
        ...
        for model in edge_candidates:
            candidates.append(ChatBackendCandidate("edge_inference", edge_base, model, edge_timeout, "edge_factual"))
        for model in desktop_priority:
        ...
        if edge_allowed:
            for model in edge_candidates:
                candidates.append(ChatBackendCandidate("edge_inference", edge_base, model, edge_timeout, "edge_degraded"))
...
def _select_research_runtime(repo_root: Path, argument: str) -> tuple[str, str]:
    ...
    if request_kind == "knowledge" and desktop_enabled:
        ...
        model = _select_available_model(edge_base, edge_candidates, provider="edge_inference")
        if model:
...
def _select_best_synthesis_model(repo_root: Path) -> tuple[str, str]:
    ...
    edge_candidates = _provider_measured_candidates(repo_root, "edge_inference", request_kind="knowledge")

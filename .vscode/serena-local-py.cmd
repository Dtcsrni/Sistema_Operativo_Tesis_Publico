...
if not exist "%SERVER_SCRIPT%" (
  >&2 echo [serena-local-py] Script no encontrado: %SERVER_SCRIPT%
  exit /b 1
)
...
if exist "%VENV_SCRIPTS_PY%" (
  "%VENV_SCRIPTS_PY%" -u "%SERVER_SCRIPT%"
  exit /b %ERRORLEVEL%
)

if exist "%VENV_BIN_PY%" (
  "%VENV_BIN_PY%" -u "%SERVER_SCRIPT%"
  exit /b %ERRORLEVEL%
)
...
where py >nul 2>nul
if not errorlevel 1 (
  py -3 -u "%SERVER_SCRIPT%"
  exit /b %ERRORLEVEL%
)

>&2 echo [serena-local-py] Python no encontrado. Se intentó: %VENV_SCRIPTS_PY%, %VENV_BIN_PY%, python, py -3
exit /b 1

...
│  │  (tasks, products, │          │  (Anthropic / proveedor de IA no publicado / etc.)    │  │
│  │   ideas, costs)    │          └──────────────────────────────────┘  │
│  └──────────────────┘                                                │
│           │                                                          │
│           ▼                                                          │
...
**Recommended path:** use Docker Compose for the consolidated SIOT stack. Mission Control no se opera fuera del stack compuesto.

### Prerequisites

- **Node.js** v18+ ([download](https://nodejs.org/))
- **OpenClaw Gateway** — `npm install -g openclaw`
- **AI API Key** — Anthropic (recommended), proveedor de IA no publicado, Google, or others via OpenRouter
...
> On Windows, this is usually `%USERPROFILE%\.openclaw\openclaw.json`.
> Mission Control also falls back to that file automatically if `OPENCLAW_GATEWAY_TOKEN` is not set.

### Operación local

La superficie soportada es Docker Compose. Abre **http://localhost:4000** después de ejecutar `docker compose up -d --build` desde la raíz del repositorio.
...
- The SIOT stack started from the repository root with Docker Compose
...
Notes:
- Keep `OPENCLAW_GATEWAY_TOKEN` in `.env` or `.env.local`; do not commit it to the repo.
- The compose network resolves `openclaw-gateway` directly; do not point Mission Control at host-local ports for the normal path.
...
|:---------|:--------:|:--------|:------------|
| `OPENCLAW_GATEWAY_URL` | ✅ | `http://openclaw-gateway:18789` | HTTP URL to OpenClaw Gateway |
...
See [PRODUCTION_SETUP.md](https://github.com/Dtcsrni/Sistema_Operativo_Tesis_Publico/blob/main/04_implementacion/control_mission/PRODUCTION_SETUP.md) for the full production guide.
...
OPENCLAW_GATEWAY_URL=http://YOUR_SERVER_IP:18789
...
```env
OPENCLAW_GATEWAY_URL=https://your-machine.tailnet-name.ts.net
OPENCLAW_GATEWAY_TOKEN=your-shared-token
```
...
MIT License — see [LICENSE](https://github.com/Dtcsrni/Sistema_Operativo_Tesis_Publico/blob/main/04_implementacion/control_mission/LICENSE) for details.

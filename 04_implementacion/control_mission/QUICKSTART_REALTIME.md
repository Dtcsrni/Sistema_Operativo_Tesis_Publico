...
```bash
cd ~/Documents/Shared/Sistema_Operativo_Tesis_Posgrado
...
No se mantiene ruta local fuera de Docker Compose para Mission Control. Si necesitas levantar el stack, usa siempre `docker compose up -d --build` desde la raíz del repositorio y deja que `OPENCLAW_GATEWAY_URL` se resuelva dentro de la red del stack.

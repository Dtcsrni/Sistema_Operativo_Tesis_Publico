...
export async function POST(
  ...
  try {
    ...
    if (otherOrchestrators.length > 0) {
      return NextResponse.json({
        error: 'Otros orquestadores disponibles',
        message: `Hay ${otherOrchestrators.length} ${otherOrchestrators.length === 1 ? 'orquestador disponible' : 'orquestadores disponibles'} en este espacio de trabajo: ${otherOrchestrators.map(o => o.name).join(', ')}. Por favor, asigne esta tarea directamente a uno de ellos.`,
        otherOrchestrators,
    ...
    const planningPrompt = `SOLICITUD DE PLANIFICACIÓN

Título de la Tarea: ${task.title}
Descripción de la Tarea: ${task.description || 'Sin descripción proporcionada'}

Estás iniciando una sesión de planificación para esta tarea. Sigue el protocolo de planificación de OpenClaw.

Genera tu PRIMERA pregunta para entender lo que el usuario necesita. Recuerda:
- Las preguntas deben ser de opción múltiple
- Incluye una opción "Otro" (Other)
- Sé específico para ESTA tarea, no genérico

Responde ÚNICAMENTE con JSON válido en este formato:
{
  "question": "¿Su pregunta aquí?",
  "options": [

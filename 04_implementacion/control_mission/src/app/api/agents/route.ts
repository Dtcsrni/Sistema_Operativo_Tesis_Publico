...
export async function GET(request: NextRequest) {
  try {
    ...
    const includeOfflineGateway = request.nextUrl.searchParams.get('include_offline_gateway') === '1';
    ...
    if (workspaceId) {
      agents = queryAll<Agent>(`
        SELECT * FROM agents
        WHERE workspace_id = ?
          AND (? = 1 OR NOT (source = 'gateway' AND status = 'offline'))
        ORDER BY is_master DESC, name ASC
      `, [workspaceId, includeOfflineGateway ? 1 : 0]);
    } else {
      agents = queryAll<Agent>(`
        SELECT * FROM agents
        WHERE (? = 1 OR NOT (source = 'gateway' AND status = 'offline'))
        ORDER BY is_master DESC, name ASC
      `, [includeOfflineGateway ? 1 : 0]);
    }

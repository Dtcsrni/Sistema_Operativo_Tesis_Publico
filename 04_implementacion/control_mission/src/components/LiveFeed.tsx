        ...
        <div className="flex items-center">
          {!mobileMode && (
            <button
              ...
              aria-label={effectiveMinimized ? 'Expandir canal' : 'Minimizar canal'}
            >
              {effectiveMinimized ? <ChevronLeft className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
            </button>
          )}
          {!effectiveMinimized && <span className="text-sm font-medium uppercase tracking-wider">Canal en Vivo</span>}
        </div>
      ...
      {!effectiveMinimized && (
        <div className="flex-1 overflow-y-auto p-2 space-y-1 pb-[calc(0.5rem+env(safe-area-inset-bottom))]">
          {filteredEvents.length === 0 ? (
            <div className="text-center py-8 text-mc-text-secondary text-sm">Sin eventos aún</div>
          ) : (

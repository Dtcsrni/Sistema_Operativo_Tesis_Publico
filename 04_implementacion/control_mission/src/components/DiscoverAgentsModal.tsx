    ...
    <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-3 sm:p-4">
      <div className="bg-mc-bg-secondary border border-mc-border rounded-t-xl sm:rounded-lg w-full max-w-2xl max-h-[88vh] sm:max-h-[80vh] flex flex-col pb-[env(safe-area-inset-bottom)] sm:pb-0">
        ...
        <div className="flex items-center justify-between p-4 border-b border-mc-border">
          <div>
              <h2 className="text-lg font-semibold flex items-center gap-2">
                ...
                Descubrir Agentes de Pasarela
              </h2>
              <p className="text-sm text-mc-text-secondary mt-1">
                Importar agentes existentes desde la Pasarela OpenClaw
              </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {loading && (
            <div className="flex items-center justify-center py-12">
              ...
              <span className="text-mc-text-secondary">Descubriendo agentes desde la Pasarela...</span>
            </div>
          )}
          ...
          {importResult && (
            <div className="flex items-center gap-3 p-4 bg-green-500/10 border border-green-500/20 rounded-lg mb-4">
              ...
              <span className="text-sm text-green-400">
                Importados {importResult.imported} agente{importResult.imported !== 1 ? 's' : ''}
          ...
          {!loading && !error && agents.length === 0 && (
            <div className="text-center py-12 text-mc-text-secondary">
              <p>No se encontraron agentes en la Pasarela.</p>
              <p className="text-sm mt-2">Asegúrate de que la Pasarela OpenClaw esté ejecutándose y tenga agentes configurados.</p>
            </div>
          )}
            ...
            <>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
                <span className="text-sm text-mc-text-secondary">
                  {agents.length} agente{agents.length !== 1 ? 's' : ''} encontrados
                  {availableCount < agents.length && ` · ${agents.length - availableCount} ya importados`}
                </span>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 text-xs text-mc-text-secondary mt-0.5">
                          ...
                          {agent.channel && <span>Canal: {agent.channel}</span>}
                          {agent.status && <span>Estado: {agent.status}</span>}
        ...
        <div className="flex items-center justify-between p-4 border-t border-mc-border">
            <span className="text-sm text-mc-text-secondary">
              {selectedIds.size > 0 ? `${selectedIds.size} seleccionados` : 'Selecciona agentes para importar'}
            </span>
          <div className="flex flex-wrap gap-2">
            <button
              onClick={onClose}

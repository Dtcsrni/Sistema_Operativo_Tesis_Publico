    ...
    return (
      <div className="min-h-screen bg-mc-bg flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-mc-text-secondary">Cargando espacios de trabajo...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-mc-bg">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl font-bold">Control de Misión</h1>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        <div className="mb-8">
          <h2 className="text-2xl font-bold mb-2">Todos los Espacios</h2>
          <p className="text-mc-text-secondary">
            Selecciona un espacio de trabajo para ver su cola de misiones y agentes
          </p>
        </div>
        ...
        {workspaces.length === 0 ? (
          <div className="text-center py-16">
            ...
            <h3 className="text-lg font-medium mb-2">Aún no hay espacios</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            >
              ...
              <span className="text-mc-text-secondary font-medium">Agregar Espacio</span>
            </button>
          </div>
        )}
      </main>
      ...
      <div className="bg-mc-bg-secondary border border-mc-border rounded-xl p-4 sm:p-6 hover:border-mc-accent/50 transition-all hover:shadow-lg cursor-pointer group relative min-h-[172px]">
        ...
        <div className="flex items-center gap-4 text-sm text-mc-text-secondary mt-4">
          <div className="flex items-center gap-1">
            ...
            <span>{workspace.taskCounts.total} tareas</span>
          </div>
          <div className="flex items-center gap-1">
            <Users className="w-4 h-4" />
            <span>{workspace.agentCount} agentes</span>
          </div>
      ...
      <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-3 sm:p-4" onClick={() => setShowDeleteConfirm(false)}>
        <div className="bg-mc-bg-secondary border border-mc-border rounded-t-xl sm:rounded-xl w-full max-w-md p-5 sm:p-6 pb-[calc(1.25rem+env(safe-area-inset-bottom))] sm:pb-6" onClick={e => e.stopPropagation()}>
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h3 className="font-semibold text-lg">Eliminar Espacio</h3>
              <p className="text-sm text-mc-text-secondary">Esta acción no se puede deshacer</p>
            </div>
          </div>
          
          <p className="text-mc-text-secondary mb-6">
            ¿Estás seguro de que deseas eliminar <strong>{workspace.name}</strong>? 
            {workspace.taskCounts.total > 0 && (
              <span className="block mt-2 text-mc-accent-red">
                ⚠️ Este espacio tiene {workspace.taskCounts.total} tarea(s). Elimínalas primero.
              </span>
            )}
          </p>
          ...
          <div className="flex justify-end gap-3">
            ...
            >
              {deleting ? 'Eliminando...' : 'Eliminar Espacio'}
            </button>
          </div>
        </div>
      </div>

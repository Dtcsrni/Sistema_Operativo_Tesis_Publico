...
export function ConvoyTab({ taskId, taskTitle, taskStatus }: ConvoyTabProps) {
  ...
  if (loading) {
    return <div className="text-center py-8 text-mc-text-secondary text-sm">Cargando convoy...</div>;
  }
        ...
        {!showCreateForm ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-mc-text-secondary mb-4">
              Divide esta tarea en sub-tareas paralelas para una finalización más rápida
            </p>
          ...
          <div className="space-y-4">
            <h3 className="text-sm font-medium">Definir Sub-Tareas</h3>
            {newSubtasks.map((st, i) => (
              <div key={i} className="space-y-2 p-3 bg-mc-bg rounded-lg border border-mc-border">
                <input
                  ...
                  placeholder={`Título de la sub-tarea ${i + 1}`}
            ...
            <div className="flex gap-2 justify-end">
              ...
              >
                {isSubmitting ? 'Creando...' : 'Crear Convoy'}
              </button>
            </div>
          </div>
        )}
    ...
    <div className="space-y-4">
      ...
      <div>
        <div className="flex items-center justify-between text-xs text-mc-text-secondary mb-1">
          <span>{progress?.completed || 0} de {progress?.total || 0} completado</span>
          <span>{pct}%</span>
        </div>
        ...
        {(progress?.failed || 0) > 0 && (
          <div className="text-xs text-red-400 mt-1">{progress!.failed} fallido</div>
        )}
      </div>
      ...
      <div className="space-y-1">
        <h4 className="text-xs font-medium uppercase text-mc-text-secondary mb-2">Sub-Tareas</h4>
          ...
          <div key={st.id} className="bg-mc-bg rounded-lg border border-mc-border/50">
            ...
            {expandedSubtasks.has(st.id) && (
              <div className="px-3 pb-3 pl-10 text-xs text-mc-text-secondary space-y-1">
                <div>ID de Tarea: <span className="font-mono text-mc-text">{st.task_id}</span></div>
                {st.depends_on && st.depends_on.length > 0 && (
                  <div>Depende de: {st.depends_on.join(', ')}</div>
                )}
              </div>
            )}
          </div>

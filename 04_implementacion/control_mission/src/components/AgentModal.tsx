      ...
      <div className="bg-mc-bg-secondary border border-mc-border rounded-t-xl sm:rounded-lg w-full max-w-2xl max-h-[92vh] sm:max-h-[90vh] flex flex-col pb-[env(safe-area-inset-bottom)] sm:pb-0">
        ...
        <div className="flex items-center justify-between p-4 border-b border-mc-border">
          <h2 className="text-lg font-semibold">
            {agent ? `Editar ${agent.name}` : 'Crear Nuevo Agente'}
          </h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-4">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full min-h-11 bg-mc-bg border border-mc-border rounded px-3 py-2 text-sm focus:outline-none focus:border-mc-accent"
                  placeholder="ej. Código y Automatización"
                />
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <label htmlFor="is_master" className="text-sm">
                  Orquestador Maestro (puede coordinar otros agentes)
                </label>
              </div>
              ...
              <div>
                <label className="block text-sm font-medium mb-1">
                  ...
                  {defaultModel && form.model === defaultModel && (
                    <span className="ml-2 text-xs text-mc-text-secondary">(Predeterminado)</span>
                  )}
                </label>
                {modelsLoading ? (
                  <div className="text-sm text-mc-text-secondary">Cargando modelos disponibles...</div>
                ) : (
                  <select
                    value={form.model}
                    ...
                    className="w-full min-h-11 bg-mc-bg border border-mc-border rounded px-3 py-2 text-sm focus:outline-none focus:border-mc-accent"
                  >
                    <option value="">-- Usar Modelo Predeterminado --</option>
                    {availableModels.map((model) => (
                      <option key={model} value={model}>
                        {model}{defaultModel === model ? ' (Predeterminado)' : ''}
                      </option>
                    ))}
                  </select>
                )}
                <p className="text-xs text-mc-text-secondary mt-1">
                  Modelo de IA utilizado por este agente. Dejar vacío para usar el predeterminado de OpenClaw.
                </p>
              </div>

              {/* Prefijo de Llave de Sesión */}
              <div>
                <label className="block text-sm font-medium mb-1">Prefijo de Llave de Sesión</label>
                <input
                  type="text"
                ...
                />
                <p className="text-xs text-mc-text-secondary mt-1">
                  Prefijo de enrutamiento de sesión de OpenClaw. Predeterminado a &quot;agente:principal:&quot; si no se establece.
                </p>
              </div>
            </div>
          )}
          ...
          {activeTab === 'soul' && (
            <div>
              <label className="block text-sm font-medium mb-2">
                SOUL.md - Personalidad e Identidad del Agente
              </label>
              <textarea
                value={form.soul_md}
                ...
                placeholder="# Nombre del Agente&#10;&#10;Define la personalidad, valores y estilo de comunicación de este agente..."
              />
            </div>
          )}
          ...
          {activeTab === 'user' && (
            <div>
              <label className="block text-sm font-medium mb-2">
                USER.md - Contexto sobre el Humano
              </label>
              <textarea
                value={form.user_md}
          ...
          {activeTab === 'agents' && (
            <div>
              <label className="block text-sm font-medium mb-2">
                AGENTS.md - Conocimiento del Equipo
              </label>
              <textarea

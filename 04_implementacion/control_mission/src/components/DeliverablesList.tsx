  ...
  const handleOpen = async (deliverable: TaskDeliverable) => {
    ...
    if (deliverable.path) {
      try {
        ...
        if (res.status === 404) {
          alert(`Archivo no encontrado:\n${deliverable.path}\n\nEs posible que el archivo haya sido movido o eliminado.`);
        } else if (res.status === 403) {
          alert(`No se puede abrir esta ubicación:\n${deliverable.path}\n\nLa ruta está fuera de los directorios permitidos.`);
        } else {
        ...
        try {
          ...
          alert(`No se pudo abrir el explorador. Ruta copiada al portapapeles:\n${deliverable.path}`);
        } catch {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-8">
        <div className="text-mc-text-secondary">Cargando entregables...</div>
      </div>
    );
  }
          ...
          <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1">
                ...
                {deliverable.deliverable_type === 'file' && deliverable.path?.endsWith('.html') && (
                  <button
                    ...
                    title="Vista previa en el navegador"
                  >
                ...
                {deliverable.path && (
                  <button
                    ...
                    title={deliverable.deliverable_type === 'url' ? 'Abrir URL' : 'Revelar en el Explorador'}
                  >
            ...
            <div className="flex items-center gap-4 mt-2 text-xs text-mc-text-secondary">
              <span className="capitalize">{
                deliverable.deliverable_type === 'file' ? 'archivo' :
                deliverable.deliverable_type === 'url' ? 'url' :
                deliverable.deliverable_type === 'artifact' ? 'artefacto' :

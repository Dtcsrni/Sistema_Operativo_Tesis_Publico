    ...
    >
      <div className="p-3 border-b border-mc-border">
        <div className="flex items-center">
          ...
          {!effectiveMinimized && (
            <>
              <span className="text-sm font-medium uppercase tracking-wider">Agentes</span>
            ...
            {activeSubAgents > 0 && (
              <div className="mb-3 mt-3 px-3 py-2 bg-green-500/10 border border-green-500/20 rounded-lg">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-mc-text">Sub-Agentes Activos:</span>
              ...
              {[
                ...
                <button
                  ...
                  className={`min-h-11 text-xs rounded uppercase ${mobileMode && isPortrait ? 'px-1' : 'px-3'} ${
                    filter === tab.id ? 'bg-mc-accent text-mc-bg font-medium' : 'text-mc-text-secondary hover:bg-mc-bg-tertiary'
                  }`}
                >
                  {tab.label}
                </button>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-mc-text-secondary truncate flex items-center gap-1">
                    ...
                    {agent.source === 'gateway' && (
                      <span className="text-[10px] px-1 py-0 bg-blue-500/20 text-blue-400 rounded" title="Importado desde Pasarela">
                        GW
                      </span>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded uppercase ${getStatusBadge(agent.status)}`}>
                    {agent.status === 'working' ? 'trabajando' : agent.status === 'standby' ? 'espera' : agent.status}
                  </span>
                </div>
              </button>
                  ...
                  >
                    {isConnecting ? (
                      ...
                      <>
                        ...
                        <span>OpenClaw Conectado</span>
                      </>
                    ) : (
                      <>
                        <ZapOff className="w-3 h-3" />
                        <span>Conectar a OpenClaw</span>
                      </>
                    )}
                  </button>
                </div>

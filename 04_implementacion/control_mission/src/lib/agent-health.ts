...
function dispatchModelStatus(activity?: ActivitySignal): 'model_unavailable' | 'awaiting_model' | 'approval_required' | undefined {
  if (!activity) return undefined;
  const metadata = parseMetadata(activity.metadata || undefined);
  const status = typeof metadata?.status === 'string' ? metadata.status : '';
  if (status === 'model_unavailable') return 'model_unavailable';
  if (status === 'model_error') return 'awaiting_model';
  if (status === 'approval_required') return 'approval_required';
  if (activity.message.toLowerCase().includes('model unavailable')) return 'model_unavailable';
  return undefined;
}
...
export function evaluateAgentHealth(agentId: string): AgentHealthEvaluation {
  ...
  if (signals.latestUserNote && !signals.assistantAfterLatestUser) {
    ...
    const latestModelStatus = parseTimestampMs(signals.latestActivity?.created_at) > parseTimestampMs(signals.latestUserNote.created_at)
      ? dispatchModelStatus(signals.latestActivity)
      : undefined;
    if (latestModelStatus === 'model_unavailable' || latestModelStatus === 'awaiting_model') {
      return buildEvaluation(
        agentId,
        'stalled',
        latestModelStatus,
        latestModelStatus === 'model_unavailable' ? 'No model available' : 'Awaiting model',
        'warning',
        latestModelStatus === 'model_unavailable'
          ? 'Mission Control delivered the task internally, but no model provider was available; the agent is not actually working.'
          : 'Mission Control delivered the task internally, but model execution failed before an assistant reply was produced.',
        0.9,
        activeTask,
        signals,
        true

...
import { evaluateAgentHealth } from '@/lib/agent-health';
...
export interface TaskFlightRecorder {
  ...
  summary: {
    ...
    chat_status: 'none' | 'answered' | 'awaiting_reply' | 'awaiting_model' | 'model_unavailable' | 'approval_required' | 'reply_not_surfaced';
    ...
    execution_evidence: {
      ...
      deliverable_count?: number;
      has_model_execution?: boolean;
      model_state?: 'no_model' | 'awaiting_model' | 'awaiting_approval' | 'executing';
    };
  };
...
export function buildTaskFlightRecorder(taskId: string): TaskFlightRecorder | null {
  ...
  const assignedAgent = task.assigned_agent_id
    ? queryOne<AgentRow>('SELECT id, name, avatar_emoji, role, model FROM agents WHERE id = ?', [task.assigned_agent_id])
    : undefined;
  ...
  const currentHealth = task.assigned_agent_id ? evaluateAgentHealth(task.assigned_agent_id) : undefined;
  ...
  const completionAfterUser = lastUserNote
    ...
    : undefined;
  const dispatchStateAfterUser = lastUserNote
    ? latestByTime(activities.filter(a => {
        if (parseTimestampMs(a.created_at) <= parseTimestampMs(lastUserNote.created_at)) return false;
        const md = parseJsonObject(a.metadata);
        const status = typeof md?.status === 'string' ? md.status : '';
        return status === 'model_unavailable' || status === 'model_error' || status === 'approval_required';
      }), a => a.created_at)
    : undefined;
  ...
  if (lastUserNote) {
    ...
    } else if (dispatchStateAfterUser) {
      const md = parseJsonObject(dispatchStateAfterUser.metadata);
      const status = typeof md?.status === 'string' ? md.status : '';
      if (status === 'approval_required') {
        chatStatus = 'approval_required';
        chatDiagnosis = 'Mission Control received an approval-required response instead of a model reply.';
      } else {
        chatStatus = status === 'model_unavailable' ? 'model_unavailable' : 'awaiting_model';
        chatDiagnosis = status === 'model_unavailable'
          ? 'Mission Control tried to dispatch internally, but no model provider was available.'
          : 'Mission Control tried to dispatch internally, but model execution failed before an assistant reply was produced.';
      }
  ...
  return {
    ...
    summary: {
      ...
      health_display_state: currentHealth?.display_state
        || (typeof healthMetadata?.display_state === 'string' ? healthMetadata.display_state : health?.health_state),
      health_reason: currentHealth?.reason
        || (typeof healthMetadata?.reason === 'string' ? healthMetadata.reason : undefined),
      // Execution evidence: provider/model/tokens/checkpoints/deliverables
      execution_evidence: (function buildEvidence() {
        const evidence: {
          provider?: string;
          model?: string;
          token_usage?: number;
        ...
        // Inspect sessions for provider/model/status hints
        for (const s of [...sessions].reverse()) {
          try {
            const maybe = (s as any).payload || (s as any).metadata || (s as any);
            if (maybe && typeof maybe === 'object') {
              if (!evidence.provider && typeof maybe.last_provider === 'string') evidence.provider = String(maybe.last_provider);
              if (!evidence.model && typeof maybe.last_model === 'string') evidence.model = String(maybe.last_model);
              if (!evidence.model_state && typeof maybe.last_status === 'string') {
                if (maybe.last_status === 'approval_required') evidence.model_state = 'awaiting_approval';
                else if (maybe.last_status === 'model_error' || maybe.last_status === 'model_unavailable') evidence.model_state = 'awaiting_model';
                else if (maybe.last_status === 'ok') evidence.model_state = 'executing';
              }
            }
          } catch {}
        }

        // Inspect activities and checkpoints for explicit engine/provider/token info
        for (const a of [...activities].reverse()) {
          const md = parseJsonObject(a.metadata);
          if (md) {
            if (!evidence.provider && typeof md.provider === 'string') evidence.provider = String(md.provider);
            if (!evidence.model && typeof md.model === 'string') evidence.model = String(md.model);
            if (!evidence.token_usage && typeof md.token_usage === 'number') evidence.token_usage = Number(md.token_usage);
            if (!evidence.model_state && typeof md.status === 'string' && md.status === 'approval_required') evidence.model_state = 'awaiting_approval';
            if (!evidence.model_state && typeof md.status === 'string' && (md.status === 'model_error' || md.status === 'model_unavailable')) evidence.model_state = 'awaiting_model';
            if (!evidence.model_state && typeof md.status === 'string' && md.status === 'ok') evidence.model_state = 'executing';
          }
        }

        for (const cp of [...checkpoints].reverse()) {
          const md = parseJsonObject(cp.context_data) || parseJsonObject(cp.state_summary);
          if (md) {
            if (!evidence.provider && typeof md.provider === 'string') evidence.provider = String(md.provider);
            if (!evidence.model && typeof md.model === 'string') evidence.model = String(md.model);
          }
        ...
        evidence.checkpoint_count = checkpoints.length;
        evidence.deliverable_count = deliverables.length;

        // Derive has_model_execution and final state
        evidence.has_model_execution = Boolean(evidence.model || (evidence.token_usage && evidence.token_usage > 0) || evidence.checkpoint_count > 0 || evidence.deliverable_count > 0);
        if (!evidence.model && assignedAgent?.model && ['assigned', 'in_progress', 'convoy_active', 'testing', 'review', 'verification'].includes(task.status)) {
          evidence.model = assignedAgent.model;
          if (!evidence.model_state) {
            evidence.model_state = 'executing';
          }
        }
        if (!evidence.model_state) {
          if (evidence.has_model_execution && evidence.model) evidence.model_state = 'executing';
          else if (!evidence.model && (evidence.checkpoint_count > 0 || evidence.deliverable_count > 0)) evidence.model_state = 'executing';
          else evidence.model_state = 'no_model';
        }

...
test('redispatch after an unanswered operator question is not treated as completion', () => {
  ...
  run(
    `INSERT INTO task_activities (id, task_id, agent_id, activity_type, message, created_at)
     VALUES (?, ?, ?, 'status_changed', 'Task dispatched to Builder - Agent is now working on this task', ?)`,
    [crypto.randomUUID(), taskId, agentId, minutesAgo(1)]
  );

  const evaluation = evaluateAgentHealth(agentId);
  assert.equal(evaluation.signals.completion_after_latest_user, false);
  assert.equal(evaluation.display_label, 'Chat reply overdue');
});

test('session heartbeat does not mask a task without real progress', () => {
  const { agentId, taskId } = seedAgentTask({ updatedMinutesAgo: 14 * 60, status: 'in_progress' });
  seedActiveSession(agentId, taskId, 1);

  const evaluation = evaluateAgentHealth(agentId);
  assert.equal(evaluation.health_state, 'stuck');
  assert.equal(evaluation.display_state, 'genuinely_stuck');
  assert.equal(evaluation.severity, 'danger');
  assert.match(evaluation.reason, /meaningful progress signal/);
});
...
test('assistant reply after latest operator question clears chat reply overdue state', () => {
  const { agentId, taskId } = seedAgentTask({ updatedMinutesAgo: 10 });
  seedActiveSession(agentId, taskId, 10);
  run(
    `INSERT INTO task_notes (id, task_id, content, mode, role, status, delivered_at, created_at)
     VALUES (?, ?, 'are you stuck?', 'direct', 'user', 'delivered', ?, ?)`,
    [crypto.randomUUID(), taskId, minutesAgo(35), minutesAgo(35)]
  ...
  run(
    `INSERT INTO task_notes (id, task_id, content, mode, role, status, delivered_at, created_at)
     VALUES (?, ?, 'No, sigo trabajando.', 'direct', 'assistant', 'delivered', ?, ?)`,
    [crypto.randomUUID(), taskId, minutesAgo(34), minutesAgo(34)]
  );

  const evaluation = evaluateAgentHealth(agentId);
  assert.equal(evaluation.signals.assistant_reply_after_latest_user, true);
  assert.notEqual(evaluation.display_label, 'Chat reply overdue');
  assert.notEqual(evaluation.display_state, 'needs_attention');
});

test('model_unavailable after latest operator question clears false working state', () => {
  const { agentId, taskId } = seedAgentTask({ updatedMinutesAgo: 2 });
  seedActiveSession(agentId, taskId, 10);
  run(
    `INSERT INTO task_notes (id, task_id, content, mode, role, status, delivered_at, created_at)
     VALUES (?, ?, 'run system diagnostics', 'direct', 'user', 'delivered', ?, ?)`,
  ...
  const evaluation = evaluateAgentHealth(agentId);
  assert.equal(evaluation.health_state, 'stalled');
  assert.equal(evaluation.display_state, 'model_unavailable');
  assert.equal(evaluation.display_label, 'No model available');
});

...
import { getOpenClawGatewayToken, getOpenClawGatewayUrl } from './config';
...
export class OpenClawClient extends EventEmitter {
  ...
  constructor(private url: string = getOpenClawGatewayUrl(), token: string = getOpenClawGatewayToken()) {
    super();
    this.token = token;
...
// Start periodic cleanup to prevent unbounded cache growth
  ...
  async call<T = unknown>(method: string, params?: Record<string, unknown>, timeoutMs = 120_000): Promise<T> {

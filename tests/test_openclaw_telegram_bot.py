...
from openclaw_local.telegram_bot import _chat_backend_candidates, _chat_request_profile, _chat_state_key, _is_greeting, _normalize_research_response, _process_update_safe, dispatch_command, dispatch_mission_control_agent_message, handle_update, parse_command, telegram_configured, web_search  # noqa: E402
...
def test_parse_command_detects_natural_research_request() -> None:
    assert parse_command("investiga el origen de la palabra chido") == ("investiga", "el origen de la palabra chido")
    assert parse_command("investígalo") == ("investiga", "investígalo")


def test_telegram_configured_accepts_token_alias(monkeypatch) -> None:
    monkeypatch.delenv("OPENCLAW_TELEGRAM_BOT_TOKEN", raising=False)
    monkeypatch.setenv("OPENCLAW_TELEGRAM_TOKEN", "alias-token")
...
def test_status_command_reports_chat_provider_and_model(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("openclaw_local.telegram_bot.probe_runtime_status", lambda repo_root: {"active_runtime": "edge_inference", "state": "npu_experimental_ready"})
    monkeypatch.setattr("openclaw_local.telegram_bot.build_preflight_report", lambda repo_root: {"status": "ok"})
    monkeypatch.setattr("openclaw_local.telegram_bot.list_available_models", lambda base_url: (True, ["mistral-nemo:12b"]))

    payload = dispatch_command("estado", "", repo_root=ROOT, store=store)

    assert payload["status"] == "ok"
    assert "runtime=edge_inference state=npu_experimental_ready" in payload["text"]
    assert "chat_provider=web_session reported_model=on_response" in payload["text"]
    assert "chat_model=gpt-5.4" not in payload["text"]


def test_status_command_degrades_when_probes_fail(tmp_path: Path, monkeypatch) -> None:
    store = OpenClawStore(tmp_path / "openclaw.db")

    monkeypatch.setattr("openclaw_local.telegram_bot.probe_runtime_status", lambda repo_root: (_ for _ in ()).throw(RuntimeError("probe caido")))
    monkeypatch.setattr("openclaw_local.telegram_bot.build_preflight_report", lambda repo_root: (_ for _ in ()).throw(RuntimeError("preflight caido")))
    ...
    assert "preflight=degraded:preflight caido" in payload["text"]
...
def test_status_command_reports_last_fallback_reason(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("openclaw_local.telegram_bot.probe_runtime_status", lambda repo_root: {"active_runtime": "edge_inference", "state": "ready"})
    monkeypatch.setattr("openclaw_local.telegram_bot.build_preflight_report", lambda repo_root: {"status": "ok"})
    monkeypatch.setattr("openclaw_local.telegram_bot.list_available_models", lambda base_url: (True, ["qwen3:4b"]))
...
def test_status_command_reports_last_backend_busy(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_list_available_models(base_url: str):
    ...
    monkeypatch.setattr("openclaw_local.telegram_bot.llamacpp_generate", fail_generate)
    monkeypatch.setattr("openclaw_local.telegram_bot.probe_runtime_status", lambda repo_root: {"active_runtime": "edge_inference", "state": "ready"})
    monkeypatch.setattr("openclaw_local.telegram_bot.build_preflight_report", lambda repo_root: {"status": "ok"})
...
def test_handle_update_falls_back_to_plain_text_when_html_delivery_fails(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("OPENCLAW_TELEGRAM_CHAT_ID", "123")
    store = OpenClawStore(tmp_path / "openclaw.db")
    captured: dict[str, str] = {}

    monkeypatch.setattr("openclaw_local.telegram_bot.send_message", lambda chat_id, text: {"status": "error", "detail": "bad_html"})
    monkeypatch.setattr(
        "openclaw_local.telegram_bot._send_plain_message",
        lambda chat_id, text: (captured.setdefault("plain_text", text), {"status": "sent", "detail": "sent"})[1],
    )
    monkeypatch.setattr("openclaw_local.telegram_bot.process_channel_text", lambda **kwargs: {"response": {"status": "ok", "text": "hola <mundo>", "model": "gpt-5.4"}, "session": {"session_id": "S1"}})
    ...
    assert "plain_text" in captured
    assert "hola <mundo>" in captured["plain_text"]
...
def test_mission_control_agent_does_not_create_apr_for_system_characteristics(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("OPENCLAW_DOMAINS_ENV_DIR", str(_write_domain_envs(tmp_path)))
    monkeypatch.setenv("OPENCLAW_TELEGRAM_EDGE_MODEL", "qwen3:4b")
    store = OpenClawStore(tmp_path / "openclaw.db")

    monkeypatch.setattr("openclaw_local.telegram_bot._deterministic_chat_response", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: {
            ...
            "semantic_status": "forced_test",
        },
    )
    monkeypatch.setattr("openclaw_local.telegram_bot.list_available_models", lambda base_url: (False, []))
    monkeypatch.setattr("openclaw_local.telegram_bot.llamacpp_ready", lambda base_url: False)

    payload = dispatch_mission_control_agent_message(
        "chat",
        "Características del sistema",
        repo_root=ROOT,
        store=store,
        chat_id="mission-control-test",
    )

    assert payload["status"] in {"model_unavailable", "model_error", "ok"}
    assert payload.get("execution_profile") == "mission_control_agent"
    assert store.list_pending_approvals() == []


def test_mission_control_execute_invokes_executor_with_task_file(tmp_path: Path, monkeypatch) -> None:
    repo_root = tmp_path
    store = OpenClawStore(tmp_path / "openclaw.db")
    calls: list[list[str]] = []

    monkeypatch.setattr("openclaw_local.telegram_bot.send_chat_action", lambda *args, **kwargs: None)
    ...
    monkeypatch.setattr("openclaw_local.telegram_bot.subprocess.run", fake_run)

    payload = dispatch_command(
        "execute",
        "TASK-001 Genera una síntesis breve",
        repo_root=repo_root,
        store=store,
        chat_id="mission-control-test",
    )
    ...
    assert payload["provider"] == "opencode-executor"
    assert calls
    assert "--task-file" in calls[0]
    assert (repo_root / "runtime" / "workspaces" / "TASK-001" / "task.json").exists()


def test_telegram_chat_keeps_approval_policy_for_ambiguous_system_phrase(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("OPENCLAW_DOMAINS_ENV_DIR", str(_write_domain_envs(tmp_path)))
    store = OpenClawStore(tmp_path / "openclaw.db")
    ...
    payload = dispatch_command("chat", "Características del sistema", repo_root=ROOT, store=store, chat_id="42")

    assert payload["status"] == "approval_required"
    assert len(store.list_pending_approvals()) == 1


def test_services_tool_lists_controlled_services(tmp_path: Path, monkeypatch) -> None:
    store = OpenClawStore(tmp_path / "openclaw.db")
    calls: list[tuple[str, str]] = []

    def fake_systemctl(action: str, service: str) -> dict[str, str]:
        calls.append((action, service))
        return {"status": "ok", "detail": "active"}

    monkeypatch.setattr("openclaw_local.telegram_bot._run_systemctl_control", fake_systemctl)

    payload = dispatch_command("herramienta", "servicios", repo_root=ROOT, store=store)

    assert payload["status"] == "ok"
    assert "openclaw-telegram-bot.service" in payload["text"]
    assert ("status", "openclaw-gateway.service") in calls


def test_approved_service_restart_executes_allowlisted_systemd(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("OPENCLAW_DOMAINS_ENV_DIR", str(_write_domain_envs(tmp_path)))
    store = OpenClawStore(tmp_path / "openclaw.db")
    calls: list[tuple[str, str]] = []
    ...
    proposal = dispatch_command("herramienta", "reinicia gateway", repo_root=ROOT, store=store, chat_id="42")
    approved = dispatch_command("aprobar", f"{proposal['approval_id']} VAL-STEP-999", repo_root=ROOT, store=store, chat_id="42")

    assert proposal["status"] == "approval_required"
    assert approved["status"] == "ok"
    assert ("restart", "openclaw-gateway.service") in calls
    assert store.list_pending_approvals() == []
    events = store.list_telegram_events(limit=1)
    assert events[0]["command"] == "service_control"
    assert events[0]["service"] == "openclaw-gateway.service"
...
def test_knowledge_chat_falls_back_to_web_evidence_when_model_is_unsupported(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("openclaw_local.telegram_bot.llamacpp_generate", lambda **kwargs: (True, "el hidalgo es un género de árboles y plantas"))
...
def test_chat_resumelo_uses_local_memory_without_model(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("openclaw_local.telegram_bot.llamacpp_generate", lambda **kwargs: (_ for _ in ()).throw(AssertionError("no debe llamar al modelo")))
...
def test_pc_native_llamacpp_uses_openai_compatible_chat_endpoint(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("OPENCLAW_DOMAINS_ENV_DIR", str(_write_domain_envs(tmp_path)))
    monkeypatch.setenv("OPENCLAW_DESKTOP_RUNTIME", "llamacpp")
    monkeypatch.setenv("OPENCLAW_DESKTOP_RUNTIME_BASE_URL", "http://pc-runtime:21435")
    monkeypatch.setenv("OPENCLAW_DESKTOP_RUNTIME_MODEL", "deepseek-r1:7b")
    monkeypatch.setenv("OPENCLAW_DESKTOP_COMPUTE_ENABLED", "1")
    store = OpenClawStore(tmp_path / "openclaw.db")
    calls: list[tuple[str, str]] = []
    ...
    def fake_llamacpp_generate(*, base_url: str, model: str, prompt: str, timeout_seconds: int = 120):
        calls.append((base_url, model))
        return True, "respuesta pc native"

    monkeypatch.setattr("openclaw_local.telegram_bot.llamacpp_generate", fake_llamacpp_generate)
...
def test_chat_backend_defaults_to_pc_docker_without_edge_fallback(monkeypatch) -> None:
    monkeypatch.delenv("OPENCLAW_CHAT_EDGE_AUTO_FALLBACK", raising=False)
    monkeypatch.setenv("OPENCLAW_DESKTOP_RUNTIME_MODEL", "deepseek-r1:7b")
    monkeypatch.setenv("OPENCLAW_DESKTOP_COMPUTE_MODEL", "deepseek-r1:7b")
    profile = {"request_kind": "standard", "complexity": "low", "channel": "mission-control", "execution_profile": "mission_control_agent"}
...
def test_chat_backend_allows_edge_only_when_assigned(monkeypatch) -> None:
    monkeypatch.delenv("OPENCLAW_CHAT_EDGE_AUTO_FALLBACK", raising=False)
    profile = {"request_kind": "standard", "complexity": "low", "target_node": "edge"}
...
def test_chat_uses_chatgpt_plus_only_when_explicitly_requested(tmp_path: Path, monkeypatch) -> None:
    ...
    def fail_llamacpp_generate(**_: object):
    ...
    monkeypatch.setattr("openclaw_local.telegram_bot.llamacpp_generate", fail_llamacpp_generate)
...
def test_chat_greeting_uses_natural_language_response(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("OPENCLAW_DOMAINS_ENV_DIR", str(_write_domain_envs(tmp_path)))
    store = OpenClawStore(tmp_path / "openclaw.db")
    warmup_calls: list[str] = []

    monkeypatch.setattr("openclaw_local.telegram_bot._check_and_start_backends_if_needed", lambda chat_id: warmup_calls.append(chat_id))
    monkeypatch.setattr("openclaw_local.telegram_bot.list_available_models", lambda base_url: (True, ["qwen3:4b"]))

    def fake_generate(*, base_url: str, model: str, prompt: str, timeout_seconds: int = 120):
        return True, "Hola, ¿en qué te ayudo hoy?"
    ...
    assert warmup_calls == ["cli"]


def test_chat_greeting_stays_heuristic_but_not_deterministic(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("OPENCLAW_DOMAINS_ENV_DIR", str(_write_domain_envs(tmp_path)))
    store = OpenClawStore(tmp_path / "openclaw.db")
    ...
    monkeypatch.setattr("openclaw_local.telegram_bot._semantic_chat_profile", fail_semantic)

    profile = _chat_request_profile("que onda", repo_root=ROOT, store=store, chat_id="chat-1")

    assert profile["intent"] == "greeting"
    assert profile["request_kind"] == "standard"
    assert profile["complexity"] == "low"
    assert profile["semantic_status"] == "heuristic_only"


def test_chat_greeting_triggers_backend_warmup(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("OPENCLAW_DOMAINS_ENV_DIR", str(_write_domain_envs(tmp_path)))
    store = OpenClawStore(tmp_path / "openclaw.db")
    warmup_calls: list[str] = []
    ...
    monkeypatch.setattr("openclaw_local.telegram_bot._check_and_start_backends_if_needed", lambda chat_id: warmup_calls.append(chat_id))
    monkeypatch.setattr("openclaw_local.telegram_bot.list_available_models", lambda base_url: (True, ["qwen3:4b"]))
    monkeypatch.setattr("openclaw_local.telegram_bot.llamacpp_generate", lambda **_: (True, "Hola, ¿en qué te ayudo?"))

    payload = dispatch_command("chat", "que onda", repo_root=ROOT, store=store, chat_id="chat-1")

    assert payload["status"] == "ok"
    assert payload["model"] == "qwen3:4b"
    assert warmup_calls == ["chat-1"]
...
def test_chat_uses_web_session_when_configured(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_web_session(prompt: str, *, timeout_seconds: int = 180):
        ...
        return True, "respuesta desde sesion web", ""
    ...
    assert payload["status"] == "ok"
    assert web_calls
    assert payload["model"] == "chatgpt_web_session"
    assert "respuesta desde sesion web" in payload["text"]


def test_ruta_command_explains_routing_without_generation(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("OPENCLAW_DOMAINS_ENV_DIR", str(_write_domain_envs(tmp_path)))
    store = OpenClawStore(tmp_path / "openclaw.db")

    def fail_generate(**_: object):
        raise AssertionError("/ruta no debe ejecutar inferencia")

    monkeypatch.setattr("openclaw_local.telegram_bot.llamacpp_generate", fail_generate)

    payload = dispatch_command("ruta", "revisa este bug de python", repo_root=ROOT, store=store)
...
def test_chat_fallback_classifies_etymology_as_research() -> None:
    profile = _chat_request_profile("investiga el origen de la palabra chido")

    assert profile["intent"] == "research"
    assert profile["request_kind"] == "knowledge"
    assert profile["complexity"] == "high"
    assert profile["semantic_status"] in {"fallback_rule", "heuristic_only"}
...
def test_chat_outcome_records_structured_trace_fields(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("openclaw_local.telegram_bot.list_available_models", lambda base_url: (True, ["qwen3:4b"]))
    monkeypatch.setattr("openclaw_local.telegram_bot.llamacpp_generate", lambda **kwargs: (True, "ok estructurado"))
...
def test_ambiguous_tools_create_approval_with_draft(tmp_path: Path, monkeypatch) -> None:
    ...
    assert image["status"] == "ok_image"
    assert image["image_caption"] == "OpenClaw imagen local"
    assert scan["status"] == "approval_required"
...
def test_image_request_executes_without_manual_apr(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("OPENCLAW_DOMAINS_ENV_DIR", str(_write_domain_envs(tmp_path)))
    ...
    assert proposal["status"] == "ok_image"
    assert approved["status"] == "approval_not_found"
    assert "Tezcatlipoca" in prompts[-1]
    assert store.list_pending_approvals() == []


def test_image_backend_unavailable_returns_clear_status_without_apr(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("OPENCLAW_DOMAINS_ENV_DIR", str(_write_domain_envs(tmp_path)))
    store = OpenClawStore(tmp_path / "openclaw.db")
    ...
    assert proposal["status"] == "image_backend_unavailable"
    assert approved["status"] == "approval_not_found"
...
def test_voice_message_uses_stt_and_tts_with_audio_reply(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: {"status": "ok", "text": "[qwen3:4b] provider=edge_inference modo=chat_local\nrespuesta corta"},
    )
    monkeypatch.setattr(

...
def test_detect_llamacpp_status_requires_a_real_ready_endpoint(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("openclaw_local.runtime_status._http_probe", lambda url, timeout=2.0: next(probes, ("degraded", 10.0, "http_404")))
...
def test_detect_llamacpp_status_uses_local_fallback_when_primary_is_down(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("OPENCLAW_DESKTOP_RUNTIME", "llamacpp")
    monkeypatch.setenv("OPENCLAW_DESKTOP_RUNTIME_BASE_URL", "http://127.0.0.1:21434")
    monkeypatch.setenv("OPENCLAW_LLAMACPP_BIND_PORT", "21435")
    monkeypatch.setenv("OPENCLAW_LLAMACPP_LOCAL_FALLBACK", "1")

    calls: list[str] = []

    def fake_http_probe(url: str, timeout: float = 2.0):
        calls.append(url)
        if url.startswith("http://127.0.0.1:21434"):
            return "degraded", 12.0, "http_503"
        return "ok", 4.0, ""

    monkeypatch.setattr("openclaw_local.runtime_status._http_probe", fake_http_probe)
    ...
    assert status["base_url"] == "http://127.0.0.1:21435"

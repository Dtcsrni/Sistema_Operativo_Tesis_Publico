...
El servidor anuncia estos nombres canónicos en `tools/list`. Por compatibilidad retroactiva también acepta aliases equivalentes con `_` en `tools/call` si un host legacy normaliza nombres.
...
- **Hosts compatibles adicionales:** Copilot, Antigravity u otros clientes MCP pueden reutilizar el mismo contrato sin requerir cambios de negocio en `serena_mcp.py`.
...
- En el workspace actual (`2026-04-12`), `serena-local` es el unico perfil publicado y su disponibilidad operativa esperada se sostiene mediante autoarranque del task HTTP al abrir la carpeta.
- En el estado actual del workspace (`2026-04-12`), `serena-local-py` no esta publicado en `.vscode/mcp.json`; su backend puede seguir sano localmente, pero eso no lo vuelve automaticamente disponible ni recomendado para los agentes del host activo.
...
- La verificacion operativa recomendada del repo es `python 07_scripts/check_serena_access.py`.
...
- El host ve las 7 herramientas esperadas.

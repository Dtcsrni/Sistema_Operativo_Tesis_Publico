...
class TestSerenaPolicy(unittest.TestCase):
    ...
    def test_append_trace_record_appends_without_rewriting_history(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            repo = Path(tmp_dir)
            minimal_repo(repo)
            ...
            trace_path.write_text(
                json.dumps({"timestamp": "2026-04-12 00:00:00", "tool": "seed"}, ensure_ascii=False) + "\n",
                encoding="utf-8",
            )

            append_trace_record(
                tool_name="canon.apply_controlled_change",
                intent="append trace test",
                minimized_inputs={"path": "docs/demo.md"},
                result={"status": "ok"},
                read_paths=["docs/demo.md"],
                write_paths=["docs/demo.md"],
                step_id="VAL-STEP-660",
                source_event_id="EVT-0001",

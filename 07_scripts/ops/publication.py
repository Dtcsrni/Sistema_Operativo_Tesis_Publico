...
def _preserve_json_blocks(text: str) -> tuple[str, dict[str, str]]:
    """Extrae bloques de script JSON y variables globales JSON para evitar que sean sanitizados."""
    preserved = {}
    counter = 0
    # Capturar scripts de tipo JSON explícito
    pattern1 = re.compile(r'<script[^>]*type\s*=\s*["\']application/json["\'][^>]*>(.*?)</script>', re.DOTALL | re.IGNORECASE)
    # Capturar asignaciones a window.SIOT_NARRATIVA (evita redactar el contenido narrativo)
    pattern2 = re.compile(r'<script>\s*window\.SIOT_NARRATIVA\s*=\s*(\{[^}]*"[^"]*":\s*"[^"]*"\s*(?:,\s*"[^"]*":\s*"[^"]*")*\})\s*;</script>', re.DOTALL | re.IGNORECASE)
    # Más simple: capturar cualquier <script> que contenga JSON complejo para SIOT_NARRATIVA
    pattern3 = re.compile(r'<script>\s*window\.SIOT_NARRATIVA\s*=.*?</script>', re.DOTALL | re.IGNORECASE)
    
    def replace_fn(match):
        nonlocal counter
        placeholder = f"__JSON_BLOCK_{counter}__"
        preserved[placeholder] = match.group(0)
        counter += 1
        return placeholder
    
    text = pattern1.sub(replace_fn, text)
    text = pattern3.sub(replace_fn, text)
    return text, preserved

def _restore_json_blocks(text: str, preserved: dict[str, str]) -> str:
    """Restaura bloques de script JSON preservados."""
    for placeholder, block in preserved.items():
        text = text.replace(placeholder, block)
    return text

def sanitize_text(text: str, config: dict) -> str:
    # Preservar bloques JSON antes de sanitizar
    text_without_json, json_blocks = _preserve_json_blocks(text)
    
    def _apply_rules(content: str) -> str:
        s = content
        for _, pattern, replacement in _regex_rules(config):
            s = pattern.sub(replacement, s)
        for literal, replacement in _literal_rules(config):
            s = s.replace(literal, replacement)
        s = FILE_URI_PATTERN.sub("[ruta_local_redactada]", s)
        s = WINDOWS_PATH_PATTERN.sub("[ruta_local_redactada]", s)
        s = re.sub(r"EVT-[A-Za-z0-9_-]+", "[evento_interno]", s)
        s = re.sub(r"EVT-\*", "[evento_interno]", s)
        s = re.sub(r"VAL-STEP-[A-Za-z0-9_-]+", "[validacion_humana_interna]", s)
        s = re.sub(r"VAL-STEP-\*", "[validacion_humana_interna]", s)
        # El bundle publico no debe conservar el literal `sha256:` porque es un token prohibido en salida.
        s = re.sub(r"sha256[:/][0-9a-fA-F]{8,64}", "[hash_redactado]", s, flags=re.IGNORECASE)
        s = re.sub(r"sha256[:/][^\s`<>()\]\[]+", "[hash_redactado]", s, flags=re.IGNORECASE)
        s = re.sub(r"sha256[:/]", "[hash_redactado]:", s, flags=re.IGNORECASE)
        s = re.sub(r'"(?:transcript_sha256|quoted_text_hash)"\s*:\s*"[0-9a-fA-F]{32,64}"', '"hash":"[redactado]"', s)
        s = re.sub(r'"screenshot_hashes"\s*:\s*\[[^\]]*\]', '"screenshot_hashes":["[redactado]"]', s)
        s = re.sub(r"(\d{4}-\d{2}-\d{2}) \d{2}:\d{2}:\d{2}", r"\1", s)
        s = re.sub(r'"content_hash"\s*:\s*"[0-9a-fA-F]{16,}"', '"content_hash":"[redactado]"', s)
        s = re.sub(r'"prev_event_hash"\s*:\s*"[0-9a-fA-FA-FINICIO]{6,}"', '"prev_event_hash":"[redactado]"', s)
        s = CURP_PATTERN.sub("[curp_redactada]", s)
        return s

    sanitized = _apply_rules(text_without_json)
    
    # Sanear también los bloques preservados de forma aislada
    sanitized_json_blocks = {}
    for placeholder, block in json_blocks.items():
        # Sanear el contenido del bloque (lo que está dentro de los tags script si aplica)
        sanitized_json_blocks[placeholder] = _apply_rules(block)

    # Restaurar bloques JSON preservados
    return _restore_json_blocks(sanitized, sanitized_json_blocks)
...
def _rewrite_public_phrases(text: str) -> str:
    ...
    for old, new in PUBLIC_PHRASE_REPLACEMENTS:
        # Usar re.sub con re.IGNORECASE para capturar variaciones de capitalización
...
def _fix_dashboard_corruption(text: str, public_rel: str) -> str:
    """Repara patrones de corrupción en archivos HTML del dashboard público.
    
    Estos patrones resultan cuando la redacción de rutas se aplica inadecuadamente
    dentro del JSON embebido en scripts.
    """
    # Solo aplicar fix en archivos HTML que probablemente contengan JSON narrativa
    if public_rel.endswith(".html"):
        # Revertir patrones pegajosos específicos causados por redacción de rutas en JSON
        fixes = {
            "basruta local no pública": "base [ruta_local_redactada]",
            "puedruta local no pública": "puede [ruta_local_redactada]",
            "soruta local no pública": "son una [ruta_local_redactada]",
            "cerruta local no pública": "cero [ruta_local_redactada]",
            "eruta local no pública": "en [ruta_local_redactada]",
            "downstrearuta local no pública": "downstream [ruta_local_redactada]",
            "dominiruta local no pública": "dominios [ruta_local_redactada]",
            "manuaruta local no pública": "manually [ruta_local_redactada]",
            "anteruta local no pública": "anterior [ruta_local_redactada]",
            "cloruta local no pública": "clo [ruta_local_redactada]",
            "Retomar estadruta local no pública": "Retomar estado [ruta_local_redactada]",
            "nocheeruta local no pública": "no cheer [ruta_local_redactada]",
        }
...
def render_public_text(*, text: str, source_rel: str, public_rel: str, config: dict, generated_at: str) -> str:
    ...
    rewritten = _fix_dashboard_corruption(rewritten, public_rel)

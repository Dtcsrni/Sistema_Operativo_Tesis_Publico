...
"$PYTHON_BIN" 07_scripts/audit/governance_gate.py --stage pre-commit
STATUS=$?
if [ $STATUS -ne 0 ]; then
  exit $STATUS
fi
...
"$PYTHON_BIN" 07_scripts/audit/governance_gate.py --stage pre-push
STATUS=$?
if [ $STATUS -ne 0 ]; then
  exit $STATUS
fi
...
"$PYTHON_BIN" 07_scripts/ops/sync_public_repo.py \
  --mode mirror \
...
"$PYTHON_BIN" 07_scripts/ops/sync_public_repo.py --mode mirror --target-dir ../Sistema_Operativo_Tesis_Publico --branch main --skip-local-mirror
"""

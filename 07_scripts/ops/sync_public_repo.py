...
def source_map_mirror(root: Path) -> dict[str, Path]:
    mapping: dict[str, Path] = {}
...
def render_payloads(source_map: dict[str, Path], *, sanitize: bool) -> dict[str, bytes]:
    ...
    for rel_path, source_path in source_map.items():
        ...
        elif sanitize and is_text_file(source_path):
            try:
...
def validate_sync_payloads(payloads: dict[str, bytes]) -> list[str]:
    ...
    if not pages_payload:
        ...
        if "refs/heads/gh-pages" not in pages_text:
            errors.append("El workflow de Pages no fija `gh-pages` como rama de despliegue protegida.")
...
def build_sync_payloads(mode: str, source_dir_value: str) -> tuple[Path, dict[str, bytes], str]:
    if mode == "bundle":
        ...
        payloads = render_payloads(source_map, sanitize=False)
    else:
        source_dir = ROOT
        source_map = source_map_mirror(ROOT)
        payloads = render_payloads(source_map, sanitize=True)

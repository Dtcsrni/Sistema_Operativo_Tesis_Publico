...
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "ops")) # ops siblings
...
def validate_public_text(root: Path) -> list[str]:
    ...
    # 1. Validar contra el manifiesto para detectar archivos huérfanos o faltantes
    ...
    if manifest_path.exists():
        try:
            ...
            for path in sorted(output_root.rglob("*")):
                ...
                if rel not in manifest_outputs:
                    errors.append(f"Archivo huérfano (no autorizado) en superficie pública: {rel}")
            
            for expected in manifest_outputs:
                if expected not in actual_files:
                    errors.append(f"Archivo faltante en superficie pública: {expected}")
        except Exception as e:
            errors.append(f"Error al procesar el manifiesto público: {e}")

...
def audit_document(file_path):
    ...
    for i, line in enumerate(lines):
        ...
        if "Pre-checks:" in line:
            # Normalizar para manejar mojibake histórico
            normalized_line = line.replace("Á‰tica", "Ética").replace("AuditorÃa", "Auditoría")
            missing_markers = [marker for marker in BASE_PRECHECK_MARKERS + RICH_PRECHECK_MARKERS if marker not in normalized_line]
            if missing_markers:
                errors.append(
...
def main():
    ...
    for directory in dirs:
        ...
        for file_path in path.glob("*.md"):
            ...
            # Omitir archivos que no siguen el patrón de fecha o son muy antiguos para el enforcement estricto
            is_legacy = False
            match_date = re.search(r"(\d{4}-\d{2}-\d{2})", file_path.name)
            if match_date:
                file_date = match_date.group(1)
                if file_date < "2026-05-01":
                    is_legacy = True
            
            file_errors = audit_document(file_path)
            if file_errors:
                # Filtrar errores de pre-checks y confirmación verbal si es legacy
                if is_legacy:
                    file_errors = [e for e in file_errors if 
                                  "no tiene bloque de autoauditoría" not in e and 
                                  "Bloque de autoauditoría incompleto" not in e and
                                  "sin bloque explícito de confirmación verbal" not in e]

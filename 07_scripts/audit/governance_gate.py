...
def safe_temp_env() -> dict[str, str]:
    ...
    # Inyectar PYTHONPATH robusto para subprocesos
    scripts_root = ROOT / "07_scripts"
    subdirs = ["ops", "audit", "utils", "benchmarks", "build_runner", "toltecayotl", "ai_tools"]
    paths = [str(ROOT), str(scripts_root)] + [str(scripts_root / d) for d in subdirs]
    
    existing_pp = env.get("PYTHONPATH", "")
    if existing_pp:
        new_pp = os.pathsep.join(paths) + os.pathsep + existing_pp
    else:
        new_pp = os.pathsep.join(paths)
...
def checks_for_stage(stage: str) -> list[tuple[str, list[str]]]:
    ...
    if stage == "pre-push":
        if should_skip_gpg_check():
            return pre_commit_checks
        return pre_commit_checks + [("Verificar firma GPG", [python, "07_scripts/utils/setup_gpg_attestation.py", "--check"])]
    if stage == "ci":
